...
export default function BDL() {
  ...
  const diasVacios = [false, false, false, false, false, false, false];
  ...
  const [registros, setRegistros] = useState<Registro[]>([
    {
      ...
      areas: {
        barrer: [...diasVacios],
        trapear: [...diasVacios],
        lavarBanquetas: [...diasVacios],
        lijarTarja: [...diasVacios],
        maquinaVending: [...diasVacios],
        limpiarTinacos: [...diasVacios],
        lavarBotesBasura: [...diasVacios],
        lavarCajasTapas: [...diasVacios],
        tuberia: [...diasVacios],
        limpiezaAnaquel: [...diasVacios],
      },
  ...
  const generarPDF = async () => {
    try {
      ...
      const headRows: (string | number | { content: string; colSpan: number; styles: Record<string, unknown> })[][] = [
        [
          {
            content: "NO. DE SEMANA",
            colSpan: 1,
            styles: { fillColor: [26, 60, 101], textColor: 255, fontStyle: "bold", halign: "center" },
          },
          ...registros.map((reg, i) => ({
            content: `SEMANA ${i + 1}`,
            colSpan: 7,
      ...
      const columnStyles: Record<number, Record<string, unknown>> = {
        0: { cellWidth: 40, halign: "left", fontStyle: "bold", fillColor: [240, 245, 250] },
      };

      const totalDias = registros.length * 7;
      for (let i = 1; i <= totalDias; i++) {
        columnStyles[i] = { cellWidth: 8, halign: "center", fontSize: 10 };
      }
  ...
  return (
    ...
    >
      ...
      {registros.map((registro, index) => (
        ...
        >
          {/* ✅ Antes: placeholder="Semana X" — Ahora: "Bitácora de Limpieza" */}
          <input
            type="text"
          ...
          <div style={{ textAlign: "left" }}>
            <strong style={{ display: "block", marginBottom: "10px", color: "#1c3853" }}>
              Registro por área y día:
            </strong>
            {AREAS_LIST.map(({ key, label }) => (
              ...
              >
                <strong style={{ display: "block", marginBottom: "6px", color: "#1c3853" }}>
                  {label}:
                </strong>
                  ...
                  {DIAS.map((dia, diaIndex) => (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          handleCheckboxChange(index, key, diaIndex, e.target.checked)
                        }
          ...
          {/* ✅ Antes: placeholder="Observaciones" — Ahora: "Actividad" */}
          <input
            type="text"
